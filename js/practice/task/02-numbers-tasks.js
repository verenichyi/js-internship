...
function getAngleBetweenVectors(x1, y1, x2, y2) {
	const aMultipliedByB = (x1 * x2) + (y1 * y2);
	const a = Math.sqrt(x1 * x1 + y1 * y1);
	const b = Math.sqrt(x2 * x2 + y2 * y2);

	return Math.acos(aMultipliedByB / (Math.abs(a) * Math.abs(b)));
}
...
function getLastDigit(value) {
	const strValueDigits = value.toString().split('');
	return +strValueDigits[strValueDigits.length - 1];
}
